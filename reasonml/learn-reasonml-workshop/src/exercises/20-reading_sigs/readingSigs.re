...
module Fraction: {
  type t;
  /*
    TODO PR explaining that these sigs need labels on the params, was really
    confusing for ages and no examples in the docs.
   */
  let create: (~numerator:int, ~denominator:int) => t;
  let value: (t) => float;
} = {
  type t = (int, int);
  let create = (~numerator:int, ~denominator:int) => (numerator, denominator);
